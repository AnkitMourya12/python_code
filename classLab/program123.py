...
def typing_effect(string,interval): 
    for letter in string:
        sleep(interval)
        sys.stdout.write(letter)
        sys.stdout.flush()
    return ""
def color_print(string,foreground_color,background_color):
    cprint(string,foreground_color,'on_'+background_color)

if __name__ == "__main__":
    main()
    os.system("color a")
    os.system("type chromePass.txt")
    typing_effect("Data Access Completed.../", 0.10)
    color_print("Data Saved Successfully!", "red", "green")
    input()